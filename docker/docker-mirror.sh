...
set_mirror(){
  ...
  then
    error "please upgrade your docker to v1.9 or later"
    exit 1
  fi
  ...
  case "$lsb_dist" in
    ...
    then
      ...
      info "You need to restart docker to take effect: sudo service docker restart"
      success "Success."
      exit 0
    fi
    ...
    then
      ...
      info "You need to restart docker to take effect: sudo systemctl restart docker "
      success "Success."
      exit 0
    else
      error "Error: Set mirror failed, please set registry-mirror manually please."
      exit 1
    fi
  ;;
    fedora)
  ...
  ;;
    ...
    if [ "$v1" -ge 16 ]; then
      ...
      info "You need to restart docker to take effect: sudo systemctl restart docker.service"
      success "Success."
      exit 0
    else
      if can_set_json; then
  ...
  error "Error: Unsupported OS, please set registry-mirror manually."
  exit 1
}
